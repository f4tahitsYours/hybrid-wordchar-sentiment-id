...
## 🔀 Cara Push ke GitHub via VSCode

### 1. Inisialisasi Git di folder project

```bash
cd path/to/sentiment-robustness-id
git init
git remote add origin https://github.com/USERNAME/sentiment-robustness-id.git
```

### 2. Buat `.gitignore` dan `requirements.txt`

Salin konten dari bagian di atas ke file masing-masing.

### 3. Struktur folder minimal yang harus ada sebelum push
...
│   └── noise_injection.py
├── results/
│   ├── plots/       ← isi dengan PNG dari Google Drive
│   └── *.csv        ← isi dengan CSV dari Google Drive
├── .gitignore
├── requirements.txt
└── README.md
```

### 4. Download output dari Google Drive

Sebelum push, download file-file ini dari Google Drive ke folder lokal:
...
# Commit
git commit -m "feat: initial commit — sentiment robustness Indonesian NLP

- 7 notebooks end-to-end pipeline
- Baseline BiLSTM vs Hybrid Word-Char
- 6 Indonesian noise types simulation
- Statistical validation (t-test + Wilcoxon)
- Robustness Score improvement: 0.9694 → 0.9874"
